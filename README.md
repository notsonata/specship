...
> **Status:** Specship v0.9 is a public source preview. It is being dogfooded and is not yet presented as production-ready.
...
If a worker's command runner cannot initialize because of a sandbox or session setup error such as `helper_unknown_error: setup refresh had errors`, it retries the exact same command once through the host's approval path. It does not switch shells, use UI automation, increase reasoning effort, or broaden the command, and ordinary command failures are never escalated. If that recovery is unavailable or fails, the parent performs one bounded fallback. This behavior is the same on Windows, macOS, and Linux.
